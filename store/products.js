...
export const mutations = {
  ...
  sortProduct: (state, type) => {
    ...
    if (type === 'Price:Low-High') {
      return state.products.sort(compare)
    }
    if (type === 'Price:High-Low') {
      return state.products.sort(compare).reverse()
    }
    ...
    function discompare(a, b) {
      if (a.offer < b.offer) return -1
      if (a.offer > b.offer) return 1
      return 0
    }
    if (type === 'Better Discount') {
      return state.products.sort(discompare).reverse()
    }
    ...
    if (type === 'denver') {
      ...
      return state.products.filter((product) => {
        ...
        if (product.category.toLowerCase() == type.toLowerCase()) {

          state.products = []
          // console.log(state.denver)
  ...
  allproducts: ( state) =>{
      ...
      console.log("harsh")
        ...
        return state.allproduct = state.products
  },

  //////////////////////////////////

  filterquery: (state, filterbrand) => {


    const arr = filterBrand.filter((val) => {
      // console.log(val.typevalue === true)
      // console.log(val.typeValue)
      // return val.typeValue === true

      // console.log(val.id === filterBrand.id)
      return val.typevalue === true
      // return  val.id === filterBrand.id3
    })
    // console.l  og(arr)
    //  arr[0].typevalue = true
    // console.log(arr[0].title)
    //     const  nname =  arr[0].mykey
    // this.$router.push({ query: nname})
    // console.log(this.filterBrand.myKey)
    ...
    this.prod.filter((item) => {
      ...
      arr.forEach((arrr) => {
        ...
        // console.log(item.title.toLowerCase() == arrr.title.toLowerCase())

        // console.log(item.title.toLowerCase())
        // console.log(arrr.title.toLowerCase())
        // console.log(item.title)
        if (item.title.toLowerCase() == arrr.title.toLowerCase()) {
          this.prod = []
          //  console.log(arrr)
          // console.log(this.prod)
          // console.log(item)
          console.log(this.prod.push(item))
          console.log(item)
          return this.prod.push(item)
    ...
    this.prod = this.productss
    // console.log(this.prod)
    // console.log(this.products)

    //  this.prod = this.prod.filter((item) =>  console.log(item.title.toLowerCase() === arr[0].title.toLowerCase()) )

    // this.prod = this.prod.filter(( item => console.log( item.title.toLowerCase() == arr[0].title.toLowerCase())))
    // console.log(this.products)
    //  this.products.forEach((item) => console.log(item.title))
    //  console.log(this.prod)
    //  console.log('hi')


      // queryObj[product.mykey] = arr.map((val) => val.id)
  ...
  filquery: (state, brand) => {

      console.log(state.allproduct)
     
      return state.allproduct.filter((product) => {
      
          state.allproduct = state.denver
          

        if (product.category.toLowerCase() == (brand.category.toLowerCase()) && (brand.typevalue == true))
   
          { 
            console.log("harshif")
            state.products = state.allproduct
            console.log(state.allproduct)
          return state.denver.push(product)

          }
                        
        else if(product.category.toLowerCase() == (brand.category.toLowerCase()) && (brand.typevalue == false))
         {
             console.log("harshelse")
           return state.denver.splice(product)
         }
         
        else
        {
           console.log("harshh")
            // state.allproduct = state.products
           return state.products
         }
    })
  },
...
export const actions = {
  ...
  productDetail: (obj, id) => {
    obj.commit("productDetail", id);
  },

  sortProduct: (obj, type) => {
    obj.commit("sortProduct", type)
  },

  filquery: (obj, brand) => {
    obj.commit("filquery", brand)
  },

  allproducts:(obj) => {
    obj.commit("allproducts")
  }
